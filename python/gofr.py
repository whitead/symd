...
def bin_atom(i, distances, positions, bin_edges, ndims, img, PBC):
    for j in range(i+1, len(positions)):
        distances[j] = pair_distance(
            positions[i], positions[j], ndims, img, PBC)
...
def region_2(rho, d):
    i4 = d**3 / 6. * (rho**2 / d ** 2 - 1) * \
        (pi / 4 - region_1_2_theta(rho, d))
    i3 = d ** 2 * rho / 3. * \
        quad(region_2_integrand, region_1_2_theta(
            rho, d), pi / 4, args=(rho, d))[0]
    return i4 + i3
...
def calc_gofr(xyz_file, box_size, bin_width=0.1, ndims=3, PBC=True):
    ...
    with open(xyz_file, 'r') as f:
        ...
        while(1):
            try:
                ...
                for i in range(natoms):
                    (newhist, bin_edges) = bin_atom(i, distances,
                                                    positions, bin_edges, ndims, box_size, PBC)
    ...
    for i in range(len(bin_edges) - 1):
        v = calc_volume(bin_edges[i+1], box_size[0] / 2.) - \
            calc_volume(bin_edges[i], box_size[0] / 2.)
...
if __name__ == "__main__":
    ...
    if(len(sys.argv) != 5):
        print "usage: [gofr.py] [input xyz file] [output file] [run_params_t_file] [bin_width]"
        exit()

    bin_width = float(sys.argv[4])
    params = read_run_params_t(sys.argv[3])
    ndims = int(params['n_dims'])
    (r, n, gofr) = calc_gofr(sys.argv[1],
                             box_size=[int(params['box_%d_size' % x])
                                       for x in range(1, ndims+1)],
                             bin_width=bin_width,
                             ndims=ndims)
